...
class EnhancedRagQuery:
    ...
    async def _format_answer_with_summary(self, search_response: Dict[str, Any], question: str, original_query: str = None) -> str:
        """
        Use Gemini summarizer to format search results
        ...
        if not results:
            return "😔 Sorry, I couldn't find any information about '{question}'."
        
        try:
            # Prepare knowledge chunks, including complete structured information
            chunks = []
            for result in results:
                chunk_data = result["chunk"]
                
                # Pass complete chunk data, including structured_data
                chunk_for_summary = {
                    "topic": chunk_data.get("topic", "Unknown topic"),
            ...
            # Way 3: Get game field from config
            ...
            # Directly return summary content (one sentence summary + detailed explanation format)
            return summary_result["summary"]
            
        except Exception as e:
            logger.error(f"Summary generation failed: {e}")
            # Fall back to friendly error message
            return "😅 Sorry, I encountered a problem while organizing information. Let me answer you in a simple way:\n\n" + self._format_simple_answer(results)
    ...
    async def query(self, question: str, top_k: int = 3, original_query: str = None, unified_query_result = None) -> Dict[str, Any]:
        ...
        Args:
            ...
            original_query: Original query (for summary)
        ...
        # If initialization fails, return fallback_to_wiki
        if not self.is_initialized or not self.vector_store:
            print(f"❌ [RAG-DEBUG] RAG system not initialized correctly, switch to wiki mode")
            return {
                "answer": "",
                "sources": [],
                "confidence": 0.0,
        ...
        try:
            print(f"🔍 [RAG-DEBUG] Starting RAG query: {question}")
            if unified_query_result:
                print(f"📝 [RAG-DEBUG] Using preprocessed unified query result:")
                print(f"   - Original query: '{unified_query_result.original_query}'")
            ...
            if self.vector_store and self.config:
                ...
                if self.enable_hybrid_search and self.hybrid_retriever:
                    print(f"🔍 [RAG-DEBUG] Using hybrid search")
                    ...
                    if self.enable_intent_reranking and self.reranker and results:
                        print(f"🔄 [RAG-DEBUG] Applying intent-aware reranking")
                    ...
                    # Format answer (using summary or original format)
                    print(f"🔍 [SUMMARY-DEBUG] Checking summary conditions (hybrid search):")
                    ...
                    if self.enable_summarization and self.summarizer and len(results) > 0:
                        print(f"💬 [RAG-DEBUG] Using Gemini summarizer to format answer")
                        answer = await self._format_answer_with_summary(search_response, question, original_query=original_query)
                    else:
                        print(f"💬 [RAG-DEBUG] Using original format to format answer")
                    ...
                    print(f"🔍 [RAG-DEBUG] Using single vector search")
                    if self.config["vector_store_type"] == "faiss":
                    ...
                    # Apply intent-aware reranking
                    if self.enable_intent_reranking and self.reranker and results:
                        print(f"🔄 [RAG-DEBUG] Applying intent-aware reranking (single vector search mode)")
                        results = self.reranker.rerank_results(
                            results, 
                            question,
                            intent_weight=self.reranking_config.get("intent_weight", 0.4),
                    ...
                    # Format answer (using summary or original format)
                    print(f"🔍 [SUMMARY-DEBUG] Checking summary conditions (single vector search):")
                    print(f"   - enable_summarization: {self.enable_summarization}")
                    print(f"   - summarizer exists: {self.summarizer is not None}")
                    print(f"   - number of results: {len(results)}")
                ...
                # Check if the vector store is not found
                if (hasattr(self, 'vector_store_path') and self.vector_store_path is None) or not self.vector_store:
                    # No corresponding vector store found, return special error flag to switch to wiki mode
                    print(f"❌ [RAG-DEBUG] Vector store not found, switch to wiki mode")
                    return {
                        "answer": "",  # Empty answer, handled by caller
                        "sources": [],
                        "confidence": 0.0,
                        "query_time": 0.0,
                        "results_count": 0,
                        "error": "VECTOR_STORE_NOT_FOUND",
                        "fallback_to_wiki": True  # Add flag to switch to wiki mode
                    }
                else:
                    # Other error cases
                    print(f"❌ [RAG-DEBUG] Vector store query failed, reason unknown")
                    return {
                        "answer": "Sorry, the guide query system encountered an issue, please try again later.",
                        "sources": [],
                        "confidence": 0.0,
                        "query_time": 0.0,
                        "results_count": 0,
            ...
            print(f"✅ [RAG-DEBUG] RAG query completed, time: {query_time:.2f} seconds")
            return response
            
        except Exception as e:
            logger.error(f"RAG query failed: {e}")
            return {
                "answer": "Sorry, an error occurred during the query, please try again later.",
                "sources": [],
                "confidence": 0.0,
                "query_time": 0.0,
                "error": str(e)
